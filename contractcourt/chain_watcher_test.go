...
func TestChainWatcherLocalForceCloseDetect(t *testing.T) {
	...
	// localForceCloseScenario is the primary test we'll use to create a
	// quickcheck test. We'll assert that for any number of state updates,
	// and if the commitment transaction has our output or not, we're able
	// to properly detect a local force close.
	localForceCloseScenario := func(t *testing.T, numUpdates uint8,
		remoteOutputOnly bool) bool {
		...
		// party's output, we'll remove Alice's with a 50% chance.
		if remoteOutputOnly {
