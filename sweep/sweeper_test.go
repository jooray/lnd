...
func TestRetry(t *testing.T) {
	...
	expectSuccess := func(c chan Result) {
		select {
		case result := <-c:
			if result.Err != nil {
				t.Fatalf("expected successful spend, but "+
					"received error %v instead", result.Err)
			}
		case <-time.After(5 * time.Second):
			t.Fatalf("no result received")
		}
	...
	expectSuccess(resultChan0)
	expectSuccess(resultChan1)
...
func TestGiveUp(t *testing.T) {
	...
	select {
	case result := <-resultChan0:
		if result.Err == nil {
			t.Fatalf("expected failed sweeper")
		}
		testLog.Infof("Expected error: %v", result.Err)
	case <-time.After(5 * time.Second):
		t.Fatalf("no result received")
	}
